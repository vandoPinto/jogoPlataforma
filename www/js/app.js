...
function inicializacao(inicio) {
  ...
  var LEVEL = ['level1', 'level2', 'level3', 'level4', 'level5', 'level6'],   // sprite image files for loading
    ...
    END = { W: ROW_HEIGHT, H: ROW_HEIGHT },                     // logical size of coin
    ...
    IMAGES = ['ground', 'ladder', 'player', 'monster', 'coins', 'end'],   // sprite image files for loading
    PLAYER = {
  ...
  function run(fase) {
    Game.Load.images(IMAGES, function (images) {
      Game.Load.json("levels/" + LEVEL[fase],
        function (level) {
          imagem = images
          setup(images, level);
          Game.run({
  ...
  function joystick() {
    ...
    joystick2.addEventListener('touchStart', function () {
      ...
      if (joystick2.deltaX() == 0 && joystick2.deltaY() == 0) {
        joystickOnKey("pulo", false);
      }
    })
    joystick2.addEventListener('touchEnd', function () {
      //console.log('up')
      if (joystick2.deltaX() == 0 && joystick2.deltaY() == 0) {
        joystickOnKey("pulo", true);
      }
    })
    //////////////////////////////////////////////////////
    setInterval(function () {
      joystick.right() ? joystickOnKey("direita", true) : joystickOnKey("direita", false)
      joystick.left() ? joystickOnKey("esquerda", true) : joystickOnKey("esquerda", false)
      joystick.up() ? joystickOnKey("cima", true) : joystickOnKey("cima", false)
      joystick.down() ? joystickOnKey("baixo", true) : joystickOnKey("baixo", false)
    }, 1 / 30 * 1000);
  }
  ...
  function joystickOnKey(key, pressed) {
    //console.log(key)
    switch (key) {
      case "direita": player.input.right = pressed; return false;
      case "esquerda": player.input.left = pressed; return false;
      case "cima": player.input.up = pressed; return false;
      case "baixo": player.input.down = pressed; return false;
  ...
  var Player = Class.create({
    ...
    updateCollisionPoint: function (point) {
      ...
      if (point.cell.end) {
        ...
        if (Game.Math.between(this.x + point.x, col2x(point.col + 0.5) - END.W / 2, col2x(point.col + 0.5) + END.W / 2) &&  // center point of column +/- End.W/2
          Game.Math.between(this.y + point.y, row2y(point.row), row2y(point.row + 1))) {
          //console.log("chegou no end")
          point.end = true;
          finalizou = true;
          newGame();
        }
  ...
  var Renderer = Class.create({
    ...
    renderEnd: function (ctx, col, y) {

      var jetPack = this.images.end,
        x = col2x(col + 0.5),
        a = 0;//Game.Math.normalizeAngle180(x2a(x) - x2a(camera.rx)),
      d = Math.floor(12 * Math.min(1, Math.abs(a / 90))),
        w = COIN.W,
        h = COIN.H,
        x0 = tx(x, tower.or),
      ...
      ctx.drawImage(jetPack, 0, d * 36, jetPack.width, 36, x1, y - h, w, h);
